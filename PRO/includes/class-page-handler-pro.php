 ...
 */
class PEIWM_Page_Handler_Pro {

	/**
	 * Instance
	 *
	 * @var PEIWM_Page_Handler_Pro|null
	 */
	...
	public function ajax_import_page() {
		...
		try {
			$page_data_raw = isset( $_POST['page_data'] ) ? wp_unslash( $_POST['page_data'] ) : ''; // phpcs:ignore we have sanitize page_data with below created custom function sanitize_page_data
			...
			// PRO: Attach matched/downloaded media to the imported page (fixes "(Unattached)" media library issue).
			...
			if ( ! empty( $existing_pages ) ) {
				...
				if ( $check_media_library ) {
					...
					if ( ! empty( $sanitized_page_data['content_images'] ) ) {
						$updated_content = $this->import_content_images_secure( $existing_page_id, $sanitized_page_data['content_images'], $existing_content, $download_missing_images, $media_match_mode, $attach_media_to_post );
					}

					if ( ! empty( $sanitized_page_data['featured_image'] ) ) {
						$this->import_featured_image_secure( $existing_page_id, $sanitized_page_data['featured_image'], $download_missing_images, $media_match_mode, $attach_media_to_post );
					}
			...
			if ( $check_media_library && ! empty( $sanitized_page_data['content_images'] ) ) {
				$updated_content = $this->import_content_images_secure( $page_id, $sanitized_page_data['content_images'], $sanitized_page_data['post_content'], $download_missing_images, $media_match_mode, $attach_media_to_post );
			}
			...
			if ( $check_media_library && ! empty( $sanitized_page_data['featured_image'] ) ) {
				$this->import_featured_image_secure( $page_id, $sanitized_page_data['featured_image'], $download_missing_images, $media_match_mode, $attach_media_to_post );
			}
	 ...
	 * @param string $match_mode Media match mode
	 * @param bool   $attach_to_post PRO: attach the resolved attachment to $page_id if currently unattached
	 * @return string Updated content
	 */
	private function import_content_images_secure( $page_id, $images_data, $content, $download_missing = false, $match_mode = 'match_and_reuse', $attach_to_post = false ) {
		...
		foreach ( $images_data as $image_data ) {
			...
			if ( $attachment_id ) {
				...
				// PRO: attach this (matched/reused) attachment to the page if it's currently unattached
				$this->maybe_attach_media_to_post( $attachment_id, $page_id, $attach_to_post );
				...
				if ( $attachment_id && ! is_wp_error( $attachment_id ) ) {
					...
					// PRO: attach this downloaded attachment to the page if it's currently unattached
					$this->maybe_attach_media_to_post( $attachment_id, $page_id, $attach_to_post );
				}
			}
		}
	 ...
	 * @param bool   $attach_to_post PRO: attach the resolved attachment to $page_id if currently unattached
	 */
	private function import_featured_image_secure( $page_id, $image_data, $download_missing = false, $match_mode = 'match_and_reuse', $attach_to_post = false ) {
		if ( ! is_array( $image_data ) || empty( $image_data['filename'] ) ) {
			return;
		}
	...
	/**
	 * Set an attachment's post_parent to the given page — PRO "Link reused media to imported page" feature.
	 *
	 ...
	 * a different post/page.
	 *
	 * @param int|WP_Error $attachment_id Attachment ID.
	 * @param int          $page_id       Page ID to attach to.
	 * @param bool         $enabled       Whether the PRO option is active for this import.
	 */
	private function maybe_attach_media_to_post( $attachment_id, $page_id, $enabled ) {
		if ( ! $enabled || empty( $attachment_id ) || is_wp_error( $attachment_id ) || empty( $page_id ) ) {
			return;
		}
