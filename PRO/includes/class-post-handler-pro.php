 ...
 */
class PEIWM_Post_Handler_Pro {

	/**
	 * Instance
	 *
	 * @var PEIWM_Post_Handler_Pro|null
	 */
	...
	public function ajax_export_posts_chunk() {
		...
		try {
			// error_log('PEIWM DEBUG [PRO]: ajax_export_posts_chunk STARTING. Offset: ' . (isset($_POST['offset']) ? $_POST['offset'] : '0') . ', Chunk: ' . (isset($_POST['chunk_size']) ? $_POST['chunk_size'] : '50'));
			...
			if ( ! empty( $selected_ids ) ) {
				...
				$in_str = implode(',', array_map('absint', $chunk_ids));
				// Bypass WP_Query entirely to prevent 3rd party plugins from exhausting memory
				$posts = $wpdb->get_results( "SELECT * FROM {$wpdb->posts} WHERE ID IN ($in_str) ORDER BY FIELD(ID, $in_str)" );
			} else {
				// Bypass WP_Query entirely to prevent 3rd party plugins from exhausting memory
				$posts = $wpdb->get_results( $wpdb->prepare(
					"SELECT * FROM {$wpdb->posts} WHERE post_type = 'post' AND post_status IN ('publish', 'draft', 'pending', 'private', 'future') ORDER BY ID ASC LIMIT %d OFFSET %d",
					$chunk_size,
					$offset
				) );
			}
	...
	public function ajax_import_post() {
		...
		try {
			...
			// TASK-008: Read media match mode from POST parameter (set by user in UI)
			...
			// PRO: Attach matched/downloaded media to the imported post (fixes "(Unattached)" media library issue).
			// This class only loads when a valid Pro license is active, but we re-verify defensively anyway.
			$attach_media_to_post = isset( $_POST['attach_media_to_post'] ) && '1' === $_POST['attach_media_to_post'];
			if ( $attach_media_to_post && class_exists( 'PEIWM_Main' ) && ! PEIWM_Main::get_instance()->is_pro_active() ) {
				$attach_media_to_post = false;
			}
			...
			if ( ! empty( $existing_posts ) ) {
				...
				if ( $check_media_library ) {
					...
					if ( ! empty( $sanitized_post_data['content_images'] ) ) {
						$updated_content = $this->import_content_images_secure( $existing_post_id, $sanitized_post_data['content_images'], $existing_content, $download_missing_images, $media_match_mode, $attach_media_to_post );
					}

					if ( ! empty( $sanitized_post_data['featured_image'] ) ) {
						$this->import_featured_image_secure( $existing_post_id, $sanitized_post_data['featured_image'], $download_missing_images, $media_match_mode, $attach_media_to_post );
					}
			...
			if ( $check_media_library && ! empty( $sanitized_post_data['content_images'] ) ) {
				$updated_content = $this->import_content_images_secure( $post_id, $sanitized_post_data['content_images'], $sanitized_post_data['post_content'], $download_missing_images, $media_match_mode, $attach_media_to_post );
			}
			...
			if ( $check_media_library && ! empty( $sanitized_post_data['featured_image'] ) ) {
				$this->import_featured_image_secure( $post_id, $sanitized_post_data['featured_image'], $download_missing_images, $media_match_mode, $attach_media_to_post );
			}
	...
	public function ajax_check_and_download_image() {
		...
		try {
			...
			$file_path = isset( $image_data['file_path'] ) ? sanitize_text_field( $image_data['file_path'] ) : ''; // TASK-003
			
			if ( empty( $filename ) ) {
				throw new Exception( esc_html__( 'No filename provided', 'post-export-import-with-media' ) );
			}

			// TASK-003: Try to find existing attachment by filename and precise file_path
			...
			// TASK-003: Pass file_path to download_and_create_attachment for date folder preservation
			...
			error_log('PEIWM ERROR [PRO]: ajax_export_posts_chunk EXCEPTION: ' . $e->getMessage());
	...
	/**
	 * TASK-003 : Add file_path to featured image export data
	 *
	...
	private function get_featured_image_secure( $post_id ) {
		...
		// TASK-003: Calculate relative file path for precise year/month matching on import
	...
	/**
	 * TASK-004 : Content verification via remote size check
	 *
	 ...
	 * @param bool   $attach_to_post PRO: attach the resolved attachment to $post_id if currently unattached
	 */
	private function import_featured_image_secure( $post_id, $image_data, $download_missing = false, $match_mode = 'match_and_reuse', $attach_to_post = false ) {
		...
		// TASK-008: Handle always_download mode - skip matching entirely
		if ( 'always_download' === $match_mode ) {
			if ( $download_missing && ! empty( $image_url ) ) {
				...
				if ( $attachment_id && ! is_wp_error( $attachment_id ) ) {
					...
					$this->maybe_attach_media_to_post( $attachment_id, $post_id, $attach_to_post );
				}
			}
			return;
		}
		...
		// TASK-004 / TASK-008 / TASK-009: Verify content size before reusing
		...
		if ( $attachment_id ) {
			...
			// PRO: attach this (matched/reused) attachment to the post if it's currently unattached
			$this->maybe_attach_media_to_post( $attachment_id, $post_id, $attach_to_post );
	...
	/**
	 * TASK-001 : Fix filename LIKE wildcard boundary to prevent substring collisions
	 * TASK-002 : Add exact file_path matching for precise, fast lookups
	 * TASK-009 : Report whether the match was via exact file_path (vs a fallback tier)
	 *
	...
	private function find_existing_attachment_by_filename( $filename, $file_path = '', &$matched_exact = false ) {
		...
		// TASK-002: Try exact file_path match first when provided (fastest, most precise)
		if ( ! empty( $file_path ) ) {
		...
		// TASK-001: Boundary-safe filename LIKE search (requires '/' before filename)
		...
		// TASK-001: Fallback for root-level files (uploads_use_yearmonth_folders disabled)
	...
	/**
	 * TASK-003 : Thread file_path through to find_existing_attachment_by_filename()
	 * TASK-004 : Add content verification before reusing matched attachment
	 * TASK-008 : Respect media match mode setting
	 *
	 ...
	 */
	private function import_content_images_secure( $post_id, $images_data, $content, $download_missing = false, $match_mode = 'match_and_reuse', $attach_to_post = false ) {
	...
	/**
	 * Set an attachment's post_parent to the given post — PRO "Attach media to post" feature.
	 *
	 * Only claims attachments that are currently unattached (post_parent = 0), so we
	 * never re-parent media that is intentionally shared with, or already attached to,
	 * a different post.
	 *
	 * @param int|WP_Error $attachment_id Attachment ID.
	 * @param int          $post_id       Post ID to attach to.
	 * @param bool         $enabled       Whether the "Attach media to post" PRO option is active for this import.
	 */
	private function maybe_attach_media_to_post( $attachment_id, $post_id, $enabled ) {
		if ( ! $enabled || empty( $attachment_id ) || is_wp_error( $attachment_id ) || empty( $post_id ) ) {
			return;
		}
		...
		// Don't steal media that's already attached to something.
	...
	/**
	 * TASK-006 : Preserve source date folder structure on downloaded images
	 * TASK-007 : Fix unsafe remove_all_filters() call (use targeted remove_filter())
	 *
	...
	private function download_and_create_attachment( $image_url, $post_id, $image_title = '', $image_alt = '', $file_path = '' ) {
		...
		// TASK-006 / TASK-007: Parse YYYY/MM from file_path and temporarily override upload_dir
		...
		if ( ! empty( $file_path ) && get_option( 'uploads_use_yearmonth_folders' ) ) {
			...
			if ( preg_match( '#^(\d{4})/(\d{2})/#', $file_path, $matches ) ) {
				...
				// TASK-007: Store closure in named variable so it can be removed specifically
		...
		try {
			...
			// TASK-007: Remove specific callback, not all filters at priority 10
